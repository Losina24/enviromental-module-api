...
export default class CouncilDatabaseHandler {
    ...
    // Logic Methods 
    public getAllCouncils(): Promise<Council[]> {
        var query = "SELECT * FROM council";
        return new Promise<Council[]>( (resolve: any, reject: any) => {
            db.getConnection((error: any, conn: any) => {
                ...
                conn.query(query, (err: any, results: any) => {
                    ...
                    let ar: any = [];
                    results.forEach((cncil:any) => {
                        let council = new Council()
                        if (results.length != 0) {
                            council.setId(cncil.id)
                            council.setName(cncil.name)
                            council.setAddress(cncil.address)
                            council.setPhone(cncil.phone_number);
                            council.setEmail(cncil.email);
                            council.setWeb(cncil.web);
                            council.setPostalCode(cncil.postal_code);
                            council.setIban(cncil.iban);
